...
class NetworkPicassoHandler(BaseHTTPRequestHandler):
    ...
    def handle_drawio_mcp_open(self, payload: dict) -> None:
        ...
        if not _mcp.is_running():
            ...
            return
        try:
            _mcp.open_diagram_in_editor(
                xml,
                filename=f"network-picasso-{diagram_type}.drawio",
                page_name=DIAGRAM_PAGE_NAMES.get(diagram_type, diagram_type.title()),
            )
        except (ConnectionError, RuntimeError) as exc:
    ...
    def handle_drawio_mcp_all_pages(self, payload: dict) -> None:
        ...
        try:
            ...
            if saved_xml:
                ...
                generated_pages = render_all_diagrams(
                    architecture,
                    style_memory=style_memory_for_project(project_path),
                )
                results = _mcp.open_all_pages(generated_pages)
                source = "generated"
        except (ConnectionError, RuntimeError) as exc:
            self.send_error_json(503, str(exc))
            return
